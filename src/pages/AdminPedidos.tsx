...
function StatusDropdown({
  ...
  const CurrentIcon = STATUS_ICONS[currentStatus] || Clock;
  const meta = STATUS_META[currentStatus] || STATUS_META.pending;

  const options = [
    { value: "pending", label: "Pendente", Icon: Clock, color: "text-yellow-400" },
    { value: "preparing", label: "Preparo", Icon: ChefHat, color: "text-blue-400" },
    { value: "delivery", label: "Entrega", Icon: Truck, color: "text-purple-400" },
    { value: "delivered", label: "Entregue", Icon: CheckCircle, color: "text-green-400" },
  ...
  return (
    <div className="relative">
      <button
        ...
        disabled={disabled}
        className="w-full flex items-center justify-between gap-2 bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.08] hover:border-white/20 rounded-lg px-2.5 py-1.5 text-white text-[10px] font-bold transition-all cursor-pointer disabled:opacity-50"
      >
        <span className="flex items-center gap-1.5 truncate">
          <CurrentIcon size={12} className={meta.color.split(" ")[1]} />
          {meta.label}
        </span>
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 mt-1.5 w-40 bg-[#161616] border border-white/[0.08] rounded-xl py-1.5 shadow-2xl z-50 animate-scale-up">
            {options.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-1.5 text-[10px] font-bold text-left hover:bg-white/[0.05] transition-colors cursor-pointer ${currentStatus === opt.value ? "text-white bg-white/[0.03]" : "text-white/60 hover:text-white"
                    }`}
                >
